...
export async function fetchLayer(name: string, namespace: string): Promise<Layer> {
  const response = await get<Layer>(
    `${import.meta.env.VITE_BACKEND_URL}/layer?name=${name}&namespace=${namespace}`,
  );

  return response.data;
}

export async function fetchMocLayer(namespace: string, name: string): Promise<Layer> {
