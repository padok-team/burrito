...
interface LayerChecklistProps {
  ...
  onSelectionChange?: (
    selectedLayers: { name: string; namespace: string }[]
  ) => void; // Updated callback prop
}
...
const LayerChecklist: React.FC<LayerChecklistProps> = ({
  ...
  const getLayerKey = (layer: Layer): string =>
    ...
    setSelectedLayers((prevSelected) =>
      prevSelected.some(
        (selectedLayer) =>
          ...
          selectedLayer.namespace === layer.namespace
      )
        ? prevSelected.filter(
            (selectedLayer) =>
              selectedLayer.name !== layer.name ||
  ...
  const handleSelectAll = () => {
    setSelectedLayers(
      layers.map((layer) => ({ name: layer.name, namespace: layer.namespace }))
    );
  };
