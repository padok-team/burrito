...
const LayerStatus: React.FC<LayerStatusProps> = ({
  ...
  const lastRunAtText = useMemo(
    () => formatHumanDate(layer?.lastRunAt),
    [layer?.lastRunAt]
  );
  ...
  return (
    <div className={className}>
      <div
        className={twMerge(
          `h-full py-3 px-6 rounded-2xl ${styles.base[theme]}`,
          layer?.isRunning && variant === 'health'
            ? `outline-solid outline-4 ${styles.isRunning[theme]}`
            : ''
        )}
      >
        <h2 className="text-sm uppercase font-semibold text-primary-600">
          {title}
        </h2>
        {variant === 'health' && (
          <div className="flex flex-col items-center justify-center h-full text-lg min-w-16">
            {layer && <Tag variant={layer.state} className="text-lg mb-4" />}
        ...
        {layer && variant === 'lastOperation' && (
          <div className="flex">
            <div className="flex flex-col min-w-40">
              <span className={twMerge('text-lg py-1 font-bold', mutedText)}>
                {' '}
                {layer.lastRun?.action &&
                  operationResult.find(
                    (op) => op.value === layer.lastRun.action
                  )?.label}
              </span>
              <span
                className={twMerge('text-sm', mutedText)}
                title={lastRunAtTitle}
              >
                {' '}
                {lastRunAtText}
              </span>
              <span className={twMerge('text-sm', mutedText)}>
                Auto Apply is{' '}
                <span className="font-semibold">
                  {layer.autoApply ? 'enabled' : 'disabled'}
                </span>
              </span>
            </div>
            <div className="flex flex-col min-w-32">
              <div className="grid grid-cols-[20%_80%] mt-4 gap-x-8">
                <span className={twMerge('text-sm text-right', mutedText)}>
                  Author:
                </span>
                <span
                  className={twMerge('text-sm truncate pr-8', mutedText)}
                  title={layer.lastRun?.author}
                >
                  {layer.lastRun?.author}
                </span>
                <span className={twMerge('text-sm text-right', mutedText)}>
                  Message:
                </span>
                <span
                  className={twMerge('text-sm truncate pr-8', mutedText)}
                  title={layer.lastRun?.message}
                >
                  {layer.lastRun?.message}
                </span>
                <span className={twMerge('text-sm text-right', mutedText)}>
                  Commit:
                </span>
                <span
                  className={twMerge('text-sm truncate pr-8', mutedText)}
                  title={layer.lastRun?.commit}
                >
                  {layer.lastRun?.commit}
                </span>
              </div>
            </div>
          </div>
        ...
        {layer && variant === 'details' && (
          <div className="flex flex-col min-w-58">
            <div className="grid grid-cols-[30%_70%] mt-4 gap-x-4">
              ...
              <span className={twMerge('text-sm truncate pr-8', mutedText)}>
                {getLayerType(layer)}
              </span>
              {layer.terragrunt && (
                <span className={twMerge('text-sm text-right', mutedText)}>
