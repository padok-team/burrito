...
func buildBurritoCmd(app *burrito.App) *cobra.Command {
	...
	cmd.PersistentFlags().StringVar(&app.Config.Storage.Redis.Hostname, "redis-host", "burrito-redis.burrito-system", "the redis host to connect to")
	cmd.PersistentFlags().IntVar(&app.Config.Storage.Redis.ServerPort, "redis-port", 6379, "the port of the redis to connect to")
	cmd.PersistentFlags().StringVar(&app.Config.Storage.Redis.Password, "redis-password", "", "the redis password")
	cmd.PersistentFlags().IntVar(&app.Config.Storage.Redis.Database, "redis-database", 0, "the redis database")
