...
var _ = Describe("Annotations", func() {
	...
	Describe("Add/Remove annotations", Ordered, func() {
		...
		It("should not return an error when adding first annotation", func() {
			err := annotations.Add(context.TODO(), k8sClient, layer, map[string]string{annotations.LastPlanRun: "AuP6pMNxWsbSZKnxZvxD842wy0qaF9JCX8HW1nFeL1I"})
			Expect(err).NotTo(HaveOccurred())
			Expect(layer.Annotations[annotations.LastPlanRun]).To(Equal("AuP6pMNxWsbSZKnxZvxD842wy0qaF9JCX8HW1nFeL1I"))
		})
		...
		It("should not return an error when removing second annotation", func() {
			err := annotations.Remove(context.TODO(), k8sClient, layer, annotations.LastApplyRun)
			Expect(err).NotTo(HaveOccurred())
			Expect(layer.Annotations).NotTo(HaveKey(annotations.LastApplyRun))
		})
	})
})
